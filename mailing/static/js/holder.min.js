...
document.getElementById("addContactButton").addEventListener("click", function() {
            ...
            contactSelectDiv.className = "contact-select mb-2";

            const selectElement = document.createElement("select");
            selectElement.className = "form-control mb-2";
            selectElement.innerHTML = `
                <option>Контакт 1</option>
                <option>Контакт 2</option>
                <!-- ... -->`;

            contactSelectDiv.appendChild(selectElement);

            const contactsContainer = document.getElementById("contactsContainer");
            contactsContainer.insertBefore(contactSelectDiv, contactsContainer.lastElementChild);
        });

function showDateFields(selectElement) {
    const datetimeField = document.getElementById("datetimeField");
    datetimeField.innerHTML = ""; // очистка предыдущего содержимого

    switch (selectElement.value) {
        case "once":
            datetimeField.innerHTML = '<input type="datetime-local" class="form-control" name="schedule_date">';
            break;
        case "daily":
            datetimeField.innerHTML = '<input type="time" class="form-control" name="schedule_time">';
            break;
        case "weekly":
        ...
        case "monthly":
            datetimeField.innerHTML = `
                <input type="time" class="form-control mb-2" name="schedule_time">
                <input type="number" class="form-control" name="schedule_date" placeholder="Число месяца" min="1" max="31">`;
            break;
        case "yearly":
            datetimeField.innerHTML = `
                <input type="time" class="form-control mb-2" name="schedule_time">
                <input type="date" class="form-control" name="schedule_date" placeholder="День и месяц">`;
            break;
    }
    datetimeField.style.display = "block"; // показываем блок с датой и временем
}